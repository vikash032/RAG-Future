...
# Memory Management
# ----------------------
def check_memory(threshold=85) -> bool:
...
# ----------------------
def enhanced_process_pdf(uploaded_file) -> List[Dict]:
    ...
    try:
        ...
        # Enhanced PDF reading with metadata
            ...
            for page_num, page in enumerate(pdf_reader.pages):
                ...
                if page_text:
                    ...
                    for sentence in sentences:
                        if len(current_chunk + sentence) > 800:  # Chunk size limit
                            if current_chunk:
                                chunks.append({
...
def enhanced_process_csv(uploaded_file) -> List[Dict]:
    ...
    try:
        ...
        for i, row in df.iterrows():
            ...
            chunks.append({
                ...
                "data_summary": {col: str(val)[:50] for col, val in row.items() if pd.notna(val)}
            })
        ...
        with st.expander("📊 CSV Data Summary", expanded=False):
            ...
            for col, info in columns_info.items():
                st.write(f"- **{col}**: {info['dtype']}, {info['null_count']} nulls, {info['unique_values']} unique values")
...
# ----------------------
def calculate_keyword_overlap(question: str, document: str) -> float:
...
def enhanced_hybrid_retrieval(question: str, max_results: int = 5, source_filter: Optional[str] = None, 
                            similarity_threshold: float = 0.3, debug: bool = False) -> Dict:
    """
    ...
    for domain, collection in shard_collections.items():
        try:
            ...
            # Dynamic fetch size based on collection size
            try:
            ...
            for doc, meta, distance in zip(docs, metas, distances):
                ...
                keyword_score = calculate_keyword_overlap(question, doc or "")
                
                # Document quality score
                doc_length = len(doc) if doc else 0
                length_score = min(doc_length / 1000, 1.0) * 0.8 + 0.2  # Favor longer, more complete docs
                ...
                final_score = (
                    similarity_score * 0.5 +      # Vector similarity (primary)
                    keyword_score * 0.3 +         # Keyword overlap
                    length_score * 0.1 +          # Document completeness
                    metadata_score * 0.1          # Metadata richness
                )
...
def smart_answer_generation(question: str, retrieved_docs: Dict, max_context_length: int = 2000) -> str:
    ...
    try:
        for domain, collection in shard_collections.items():
            try:
                ...
                # Get sample documents for source analysis
                if count > 0:
...
# ----------------------
def enhanced_ingest_data(data: List[Dict], data_type: str = "pdf", source_name: Optional[str] = None) -> bool:
    ...
    try:
        # Validation
        if data_type not in DOMAIN_SHARDS:
        ...
        with st.expander("📊 Ingestion Summary", expanded=False):
            ...
            col1.metric("📄 Chunks Processed", total_chunks)
            col2.metric("💾 Collection", shard)
            col3.metric("🔤 Total Characters", sum(len(text) for text in texts))
...
def enhanced_chat_with_document(query: str, source_name: str, chat_history: List[Dict]) -> List[Dict]:
    ...
    with st.spinner(f"🔍 Searching in '{source_name}'..."):
        results = enhanced_hybrid_retrieval(
            query, 
            max_results=3, 
    ...
    if results["documents"]:
        ...
        response = smart_answer_generation(query, results, max_context_length=1500)
...
# ----------------------
# Arxiv and Clinical Research (Enhanced Stubs)
# ----------------------
def enhanced_fetch_arxiv_papers(query: str, max_results: int = 5) -> List[Dict]:
    """Enhanced Arxiv paper fetching (placeholder for real implementation)"""
    try:
        # This is a placeholder - implement real Arxiv API integration
        papers = []
...
def enhanced_fetch_clinical_trials(condition: str = "cancer", max_results: int = 5) -> List[Dict]:
    """Enhanced clinical trials fetching (placeholder for real implementation)"""
    try:
        trials = []
        ...
        for i in range(min(max_results, 4)):
            trials.append({
                ...
                "estimated_completion": (datetime.now() + timedelta(days=365*(2-i))).strftime("%Y-%m-%d"),
                "participants": (100 + i*50),
                "locations": [f"City {i+1}", f"City {i+2}"],
                "intervention": f"Novel {condition} therapy approach",
                "text": f"Clinical Trial: {condition.title()} Treatment Study - This randomized controlled trial investigates novel therapeutic approaches for {condition} treatment with estimated enrollment of {100 + i*50} participants."
            })
        return trials
    except Exception as e:
        st.error(f"❌ Clinical trials fetch error: {e}")
        return []
            ...
            if uploaded_file:
                ...
                if replace_existing or file_hash != st.session_state.file_hashes.get(uploaded_file.name):
                    if st.button("🚀 Process PDF", type="primary"):
                        ...
                        chunks = enhanced_process_pdf(uploaded_file)
                        if chunks:
                            if enhanced_ingest_data(chunks, "pdf", uploaded_file.name):
            ...
            if uploaded_file:
                ...
                if replace_existing or file_hash != st.session_state.file_hashes.get(uploaded_file.name):
                    if st.button("🚀 Process CSV", type="primary"):
                        ...
                        chunks = enhanced_process_csv(uploaded_file)
                        if chunks:
                            if enhanced_ingest_data(chunks, "csv", uploaded_file.name):
                                st.session_state.file_hashes[uploaded_file.name] = file_hash
    ...
    with tab3:
        ...
        if st.button("🚀 Fetch Research Papers", type="primary"):
            with st.spinner("📚 Searching Arxiv for relevant papers..."):
                papers = enhanced_fetch_arxiv_papers(research_query, max_papers)
    ...
    with tab4:
        ...
        if st.button("🚀 Search Clinical Trials", type="primary"):
            with st.spinner("🏥 Searching for clinical trials..."):
                trials = enhanced_fetch_clinical_trials(condition_query, max_trials)
        ...
        if st.button("🚀 Search Knowledge Base", type="primary") and query:
            with st.spinner("🔍 Searching through documents..."):
                ...
                if results["documents"]:
                    ...
                    st.markdown(smart_answer_generation(query, results))
                else:
